...
const SHOP_DATA = [
  ...
  ["CoffeeSupreme","drink","liverary",1.53,12.74,5.77,0.57,16],
  ...
  ["The Source Diner","food","eatbeat-ichi",38.5,83.17,6.27,0.57,14],
  ...
  ["musica print works","art","yuenchi-market2",61.33,8.29,9.22,0.57,11],
...
/* 会場図上のエリア輪郭ポリゴン（正規化%座標の頂点列）。
   フジイさんからの囲み画像をもとに順次追加。定義済みのエリアは、
   ショップ選択時にこのポリゴンを会場図上に色付き表示する。 */
const ZONE_POLY = {
  // エリア編集モード（URLに #editareas）で会場図上に描いた輪郭をここに登録。
};

/* 会場マップ上の「エリア名ラベル」の位置（公式マップPDFから抽出した
   テキスト外接矩形・正規化%）。形式: [x%, y%, 幅%, 高さ%]。
   ショップ選択時、このエリア名を会場マップ上で丸く囲んで示す。 */
const ZONE_LABEL = {
  ...
  'yuenchi-market': [61.3,1.1,7.4,1.0],
